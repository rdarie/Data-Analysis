...
do
  ...
  ## ##
  # can get rid of the individual Block00x files now that we have the assembled one
  python './shuttleFilesToFromScratch.py' --exp=$EXP --searchTerm="Block0*_ma_*.pickle" --fromScratchToData --moveItems --filesIncluded ${KEYPROMPT}
  python './shuttleFilesToFromScratch.py' --exp=$EXP --searchTerm="Block0*_ma.h5" --fromScratchToData --moveItems --filesIncluded ${KEYPROMPT}
  python './shuttleFilesToFromScratch.py' --exp=$EXP --searchTerm="Block0*_ma_*_meta.pickle" --fromScratchToData --moveItems --filesIncluded ${KEYPROMPT}
  ## ##
  ## ## iters=(ca cb ccm ccs cd)
  ## ## for ITER in "${iters[@]}"
  ## ## do
