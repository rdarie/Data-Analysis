...
if __name__ == '__main__':
        ...
        for rhsMaskIdx in range(rhsMasks.shape[0]):
            ...
            for targetName in rhGroup.columns:
                ...
                for foldIdx in range(cvIterator.n_splits + 1):
                    ...
                    '''
                    if True:
                        plotPred = predictionPerSource.xs(9, level='trialUID').reset_index()
                        plt.plot(plotPred['bin'], plotPred['ground_truth'], label='ground_truth')
                        plt.plot(plotPred['bin'], plotPred['prediction'], label='prediction')
                        for cN in plotPred.columns:
                            if cN in fullDesignColumnInfo['term'].to_list():
                                plt.plot(plotPred['bin'], plotPred[cN], label=cN)
                        plt.legend()
                        plt.show()'''
    ...
    print('Loaded and saved full predictions')
    print('\n' + '#' * 50 + '\n{}\n{}\nComplete.\n'.format(dt.now().strftime('%Y-%m-%d %H:%M:%S'), __file__) + '#' * 50 + '\n')
