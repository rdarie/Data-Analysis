...
with PdfPages(pdfPath) as pdf:
    ...
    if relativeStatsDF.groupby(['xCoords', 'yCoords']).ngroups > 1: # histogram t stats
        ...
        thisPalette = pd.Series(sns.color_palette('Set1', 2), index=['trialAmplitude', 'trialAmplitude:trialRateInHz'])
        g = sns.catplot(
            y='coef',
            x='freqBandName',
            order=thisFreqBandOrder,
            col='electrode',
            hue='names', hue_order=thisPalette.index.to_list(), palette=thisPalette.to_dict(), color='w',
            data=plotAmpStatsDF.loc[thisMaskAmp & plotAmpStatsDF['reject'], :],  #
            kind='box', whis=np.inf, saturation=0.)
        for elecName, ax in g.axes_dict.items():
            subSetMask = thisMaskAmp & (plotAmpStatsDF['electrode'] == elecName) & (~plotAmpStatsDF['reject'])
            sns.stripplot(
                data=plotAmpStatsDF.loc[subSetMask, :], ax=ax,
                y='coef', x='freqBandName',
                order=thisFreqBandOrder, hue='names', hue_order=thisPalette.index.to_list(), palette=thisPalette.to_dict(),
        ...
        thisPalette = pd.Series(sns.color_palette('Set1', 2), index=[50., 100.])
        g = sns.catplot(
            y='hedges',
            x='freqBandName',
            order=thisFreqBandOrder,
            col='electrode',
            hue='trialRateInHz', hue_order=thisPalette.index.to_list(), palette=thisPalette.to_dict(),
            data=plotRelativeStatsDF.loc[thisMaskRel & plotRelativeStatsDF['reject'], :],  #
            kind='box', whis=np.inf, saturation=0.)
        for elecName, ax in g.axes_dict.items():
            subSetMask = thisMaskRel & (plotRelativeStatsDF['electrode'] == elecName) & (~plotRelativeStatsDF['reject'])
            sns.stripplot(
                data=plotRelativeStatsDF.loc[subSetMask, :], ax=ax,
                y='hedges', x='freqBandName',
                order=thisFreqBandOrder, hue='trialRateInHz', hue_order=thisPalette.index.to_list(), palette=thisPalette.to_dict(),
