...
if correctMultiCompHere:
    ...
    reject, pval = pg.multicomp(pvalsCatDF['pval'].to_numpy(), alpha=confidence_alpha)
...
with PdfPages(pdfPath) as pdf:
    ...
    if arguments['plotThePieces']:
        widthRatios = [3] * np.unique(testGroupPieces[colName]).shape[0] + [2]
    ...
    if True:
        ...
        thisMaskAmp = plotAmpStatsDF['kinematicCondition'].notna()
        ...
        for name, ax in g.axes_dict.items():
            ...
            subSetMask = thisMaskAmp & (plotAmpStatsDF['kinematicCondition'] == kinName) & (plotAmpStatsDF['electrode'] == elecName) & (~plotAmpStatsDF['reject']) & plotAmpStatsDF['coef'].notna()
            if subSetMask.any():
                sns.stripplot(
