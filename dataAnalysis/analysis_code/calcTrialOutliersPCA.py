...
def calcPCs(
    ...
    pca_n_components = min(
        n_components, partitionData.shape[0])
    result = pd.DataFrame(
        0,
    ...
    pcs = projector.fit_transform(partitionData)
    result.loc[:, list(range(pca_n_components))] = pcs
    # print('result shape is {}'.format(result.shape))
    result = pd.concat(
        [result, partition.loc[:, ~dataColMask]],
        axis='columns')
    # result.name = 'PCs'
    result.columns.name = 'PCs'
    return result
...
def calcCovMat(
    ...
    if useMinCovDet:
        ...
        est = EmpiricalCovariance()
        try:
            est.fit(partitionData.to_numpy())
        except Exception:
            pdb.set_trace()
    result = pd.DataFrame(
...
if __name__ == "__main__":
    ...
    if not mahalDistLoaded:
        ...
        pcsDF = ash.splitApplyCombine(
            dataDF, fun=calcPCs, resultPath=resultPath,
            funKWArgs=pcOpts,
        ...
        # calc mahalDist for DF with unstacked bins
    ...
    print('#######################################################')
    refInterval = chi2.interval(1 - 1e-6, len(pcsDF.columns))
    print('Data is {} dimensional'.format(len(pcsDF.columns)))
    ...
    outlierTrials = findOutliers(
        ...
        nDim=len(pcsDF.columns), devQuantile=0.95, twoTailed=twoTailed)
    ...
    if arguments['plotting'] and outlierTrials['rejectBlock'].astype(bool).any():
        ...
        for idx, (name, row) in enumerate(theseOutliers.items()):
            outlierDataMasks = []
            for lvlIdx, levelName in enumerate(theseOutliers.index.names):
                outlierDataMasks.append(pcsDF.index.get_level_values(levelName) == name[lvlIdx])
            fullOutMask = np.logical_and.reduce(outlierDataMasks)
            for cN in pcsDF.columns:
                emgAx.flat[idx].plot(
    ...
    #         bla.to_numpy())
    #     bla = (pcsDF.xs(992, level='originalIndex').xs(3, level='segment'))
    #     ax[1].plot(
    #         bla.index.get_level_values('bin').to_numpy(),
    #         bla.to_numpy())
    #     plt.show()
