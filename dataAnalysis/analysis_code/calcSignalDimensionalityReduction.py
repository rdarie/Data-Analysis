...
if __name__ == '__main__':
    ...
    if estimatorName == 'pca':
        estimatorClass = PCA
        estimatorKWArgs = dict(n_components='mle')
        get_n_components = lambda x: x.n_components_
    elif estimatorName == 'ica':
        estimatorClass = tdr.ScoredFastICA
        estimatorKWArgs = dict(n_components=10, max_iter=1000)
        get_n_components = lambda x: x.components_.shape[0]
    joblibBackendArgs = dict(backend='loky')
    crossvalKWArgs = dict(
        cv=cvIterator,
