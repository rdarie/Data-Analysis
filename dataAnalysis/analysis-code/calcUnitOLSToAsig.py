...
def calcUnitRegressionToAsig():
    ...
    targetScalers = [(StandardScaler(), colName) for colName in targetDF.columns]
    import warnings
    ...
    tdr = TargetedDimensionalityReduction(
        featuresDF=featuresDF, targetDF=targetDF, model=sm.OLS,
        modelKWargs={'hasconstant': addInterceptToTDR},
    ...
    if arguments['plotting']:
        ...
        plt.show()
        '''
        rsquared = pd.DataFrame([{'unit': i['unit'], 'rsquared': i['reg'].rsquared} for i in tdr.regressionList])
        ...
        plt.plot(targetDF[unitName].values, label='original')
        plt.plot(prediction, label='prediction')
        plt.title('{}: R^2 = {}'.format(unitName, rsquared.loc[rsquared['rsquared'].idxmax(), 'rsquared']))
        plt.xlabel('samples')
