...
low = 250 / nyq
high = 5e3 / nyq
b, a = signal.bessel(3, [low, high], btype='band')
...
for elecGroupIdx, elec_ids in enumerate(all_elec_ids):
    for idx, timeStart in enumerate(timeRanges):
        ...
        print('data batch: %d, time points: %d to %d' % (idx, timeStart, timeMax))

        subDir = dataDir + '/KiloSort/' + fileName + '_' + elecGroupNames[elecGroupIdx]
        if not os.path.exists(subDir):
            os.makedirs(subDir)

        currData = nsxFile.getdata(elec_ids, timeStart, timeMax - timeStart)
        ...
        #plt.show()
        if idx == 0:
            with open(subDir + '/' + fileName + '_' + elecGroupNames[elecGroupIdx] + '.dat', 'wb') as fid:
                exportDataInt.astype('int16').tofile(fid)
        else:
            with open(subDir + '/' +  fileName + '_' + elecGroupNames[elecGroupIdx] + '.dat', 'ab') as fid:
                exportDataInt.astype('int16').tofile(fid)
