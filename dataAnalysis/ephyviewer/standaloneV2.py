...
from ephyviewer.tools import get_dict_from_group_param
...
class StandAloneViewer(MainViewer):
    ...
    def load_dataset(self, neo_rawio_class=None, file_or_dir_names=[], io_params={}):
        ...
        self.neorawio = []
        for rawIdx, thisPath in enumerate(file_or_dir_names):
            kargs = dict()
            if neo_rawio_class.rawmode.endswith('-file'):
                kargs['filename'] = thisPath
            elif neo_rawio_class.rawmode.endswith('-dir'):
                kargs['dirname'] = thisPath
            kargs.update(io_params)

            self.neorawio.append(neo_rawio_class(**kargs))
            self.neorawio[rawIdx].parse_header()
            print(self.neorawio[rawIdx])
        ...
        allSources = dict()
        for rawIdx, thisPath in enumerate(file_or_dir_names):
            sources = get_sources_from_neo_rawio(self.neorawio[rawIdx])
            for key, value in sources.items():
                if key not in allSources:
